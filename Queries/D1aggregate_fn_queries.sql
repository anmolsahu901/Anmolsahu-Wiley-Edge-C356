...
SELECT productLine,AVG(buyPrice) AS average_buy_price FROM products GROUP BY productLine;
...
GROUP BY productLine ORDER BY productLine;
...
SELECT productCode , SUM(priceEach*quantityOrdered) AS total FROM orderdetails  
...
SELECT productLine,MAX(buyPrice)  FROM products
 GROUP BY productLine ORDER BY MAX(buyPrice) DESC;
...
SELECT productLine,MIN(buyPrice) AS lowest_price FROM products
GROUP BY productLine ORDER BY lowest_price;
...
---FORMAT
SELECT FORMAT(AVG(buyPrice),2)
FROM products;

----using having clause
SELECT productLine ,AVG(buyPrice) 'Average Price'
FROM products
GROUP BY productLine
HAVING AVG(buyPrice) >50;


-- AVERAGE BUY PRICE OF PRODUCT LINE
-- CALCULATE AVERAGE BUY PRICE OF AVERAGE BUY PRICE OF PRODUCT LINE
...
-- PL_avg :- 45,56,78,34,NULL ,89 (average function ignores the null value)
